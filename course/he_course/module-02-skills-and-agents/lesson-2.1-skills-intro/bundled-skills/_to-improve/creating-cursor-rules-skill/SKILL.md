...
description: Expert guidance for Cursor 2.x project rules (.cursor/rules/*.mdc), migration from .cursorrules, and worked examples for Next/React-style stacks.
---
...
> 4. You are **porting** a Claude Code skill into **auto-attached** or **agent-requested** rules.  
> 5. **Do not** use for one-off instructions—those belong in chat or a single prompt, not a rule file.

# Creating Cursor Rules

You are an expert at creating effective **Cursor project rules** (`.cursor/rules/*.mdc`) that help AI assistants understand project conventions and produce better code.

## Cursor 2.x: where rules live

| Mechanism | Location | When to use |
|-----------|----------|-------------|
| **Project rules (preferred)** | `.cursor/rules/*.mdc` | Team-visible, glob-based attachment, version in Git |
| **Legacy project file** | `.cursorrules` at repo root | Still supported; prefer `.mdc` when rules grow past ~1 screen |
| **User rules** | Cursor **Settings → Rules** | Personal preferences, not committed |

If the repo only has `.cursorrules`, suggest **splitting** into 2–4 `.mdc` files by concern (e.g. `stack.mdc`, `api.mdc`, `tests.mdc`) with `globs` so context stays small.

## Course worked example (Next.js / React + TypeScript)

Use this as a **starting** `.cursor/rules/web-app.mdc` (adjust paths to the student’s app). File content:

```
---
description: Next.js App Router, TypeScript, Tailwind — default conventions for app and components
globs: ["app/**/*", "components/**/*", "src/**/*"]
alwaysApply: false
---

## Stack
- Next.js App Router, TypeScript strict, Tailwind for UI

## Components
- Prefer Server Components; add "use client" only for interactivity, browser APIs, or hooks
- Named exports for components; colocate small types next to the component

## Data & env
- No secrets in client bundles; use env only on server or via validated public `NEXT_PUBLIC_*`
```

Students should **trim** sections they do not use (e.g. no Next.js → replace with Vite + React).
