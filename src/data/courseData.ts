...
בשיעורים הבאים במודול 1 נעבור על מצבי צ'אט (Ask, Agent, Plan, Debug) ועל הגדרות Cursor לפני שמסיימים את המודול. אחר כך, במודול 2, מתחילים לבנות עם Skills.`,
        ...
        },
        {
          ...
          badge: 'BEGINNER',
          notes: [
            'מתי Ask לעומת Agent לעומת Plan לעומת Debug; מסגרת החלטות קצרה לפני שמבזבזים טוקנים.',
            'Plan mode (למשל Shift+Tab): לחשוב ולתכנן לפני בנייה; דמו משולב Plan→Debug כשיש תקלה.',
          ],
          prompts: [
            'אני בשיעור מצבי צ׳אט ב-Cursor. תן טבלה קצרה: מתי Ask, מתי Agent, מתי Plan, מתי Debug — עם דוגמת פרומפט אחת לכל מצב.',
            'תאר זרימת עבודה אחת למשימה אמיתית: התחלה ב-Ask או Plan, מעבר ל-Agent לבנייה, וסיום ב-Debug אם יש שגיאה.',
          ],
          checklist: [
            'זיהוי Ask / Agent / Plan / Debug בממשק שלכם',
            'משימה אחת: נסו ניסוח דומה ב-Plan לעומת Agent והשוו תוצאה',
            'תרגלו מעבר מצב בלי לאבד הקשר (מקלדת או תפריט — לפי הגרסה אצלכם)',
          ],
        },
        {
          id: '1.7',
...
אחרי שסיימתם, יש לכם בסיס להמשיך לשרשרת הסוכנים: PRD, ארכיטכטורה, UX — כל אחד עם הסקיל שלו.`,
        ...
        },
        {
          ...
          prompts: [
            'אתה Product Agent. מטרה: להפוך רעיון ל-PRD קצר. קלט: רעיון גולמי + קהל יעד משוער. פלט: Problem, Users, V1 scope, User flow, Success metrics, Acceptance criteria, Risks, Open questions. אם חסר מידע, שאל עד 5 שאלות קצרות לפני הכתיבה.',
            'אתה Product Agent. הנה הרעיון: [מלא כאן]. לפני כתיבת PRD, שאל רק שאלות שישנו Scope, משתמשים או הצלחה. אחר כך כתוב PRD של עמוד אחד מקסימום.',
            'אתה Product Agent. קלט: @brief.md. פלט: מה בונים, למי זה, איך יודעים שסיימנו, מה לא נכלל ב-V1, ואילו שאלות פתוחות נשארו.',
          ],
          checklist: [
        ...
        },
        {
          ...
          prompts: [
            'אתה Tech Lead Agent. מטרה: לתרגם PRD לארכיטקטורה ברת-ביצוע. קלט: @prd-v0.md + @stack.mdc אם קיים. פלט: stack אחד מומלץ, גבולות מערכת, מבנה תיקיות, ממשקים מרכזיים, ו-5 צעדי מימוש לפי סדר. אל תרחיב Scope מעבר ל-V1.',
            'אתה Tech Lead Agent. קלט: @prd-v0.md. פלט: Architecture handoff קצר למפתח: layers, key files, contracts בין שכבות, וה-slice הראשון שכדאי לבנות השבוע.',
            'אתה Tech Lead Agent. אל תיתן לי רשימת אופציות. בחר פתרון אחד ל-V1, נמק בקצרה, ואז פרט איפה כל אחריות יושבת במערכת.',
          ],
          checklist: [
        ...
        },
        {
          ...
          prompts: [
            'אתה UX Agent. מטרה: לפרק את החוויה למסכים ומצבים. קלט: @prd-v0.md + @architecture-handoff.md אם יש. פלט: User flow מסך-אחר-מסך, מה המשתמש רואה ולוחץ, ומה קורה ב-loading, empty, error בכל מסך חשוב.',
            'אתה UX Agent. בחר flow מרכזי אחד מתוך @prd-v0.md ופרק אותו למסכים, פעולות, החלטות ו-state transitions. בכל מסך פרט Success, Empty, Error.',
            'אתה UX Agent. פלט: UX handoff פרקטי ל-UI בלי עיצוב ובלי Figma. תאר מה המשתמש רואה, מה הוא עושה, ומה יכול להשתבש.',
          ],
          checklist: [
        ...
        },
        {
          ...
          prompts: [
            'אתה UI Agent. מטרה: להפוך UX handoff לתוכנית UI שמפתחים יכולים לממש. קלט: @ux-flow.md. פלט: Component list, design tokens בסיסיים, UI states, mobile vs desktop notes, ו-3 דרישות נגישות. אל תייצר CSS חד-פעמי בלי שם לרכיב או לטוקן.',
            'אתה UI Agent. קלט: @ux-flow.md. פלט: UI handoff ישים: components, variants, tokens, loading/empty/error, ונקודת נגישות אחת לכל מסך חשוב.',
            'אתה UI Agent. בחר את המסך הראשון שצריך לבנות, ותן פירוק ל-components חוזרים, props עיקריים, ומה חייב להיות reusable.',
          ],
          checklist: [
        ...
        },
        {
          ...
          badge: 'INTERMEDIATE',
          notes: [
            'לפני קוד: מצרפים PRD, ארכיטקטורה, UX ו-UI — ומבקשים סדר בנייה או תוכנית חיתוכים (slices).',
            'פלט ממוקד: docs/implementation-plan.md או docs/slice-01.md, slice-02… עם סדר ברור וחסמים לפני מימוש.',
            'חסמים (כולל אבטחה או תלות) עולים כאן לפני שמבזבזים זמן על קוד שגוי.',
          ],
          prompts: [
            'עברו ל-Plan mode. קלט: @prd-v0.md @architecture-handoff.md @ux-flow.md @ui-handoff (או שמות המסמכים שלכם). פלט: תוכנית מימוש אחת מסודרת או 2–4 חיתוכים לפי סדר, עם מה בכל slice ומה נשאר מחוץ ל-slice הראשון.',
            'אתה תכנון מימוש. מטרה: לזהות חסמים לפני קוד. קלט: כל מסמכי התכנון שיש. פלט: רשימת חסמים (טכניים, מוצר, אבטחה, תלויות), והמלצה מה לפתור לפני Composer.',
            'בקשו מהסוכן ב-Plan mode להגדיר רק את Slice 1 עד רמת קבצים והתנהגות שניתן לממש בלי ניחוש.',
          ],
          checklist: [
            'Plan mode פעיל; כל מסמכי התכנון מצורפים ב-@',
            'קובץ אחד ברור: docs/implementation-plan.md או docs/slice-01.md (ונוספים לפי הצורך)',
            'Slice 1 מוגדר מספיק כדי לפתוח שיעור 2.9 (Composer) בלי לנחש',
          ],
        },
        {
          id: '2.9',
          ...
          prompts: [
            'אתה Builder Agent. מטרה: לבנות vertical slice אחד קטן בלבד. קלט: @prd-v0.md + @architecture-handoff.md + @ux-flow.md + @project.mdc + @stack.mdc. לפני קוד: תן 4 bullets של behavior, files, risk, verification. אחר כך הצע diff קטן ורשימת בדיקות.',
            'אתה Builder Agent. אל תבנה את כל המערכת. בחר slice שאפשר לבדוק בדקות, פרט מה המשתמש יוכל לעשות בסוף הסלייס, ורק אז הצע קוד.',
            'אתה Builder Agent. קלט: @architecture-handoff.md @ui-handoff.md. פלט: Implementation plan קצר, diff קטן, ואיך להריץ ולבדוק מקומית. עצור אם חסר data contract ברור.',
          ],
          checklist: [
        ...
        },
        {
          ...
          prompts: [
            'אתה Review Agent. מטרה: למצוא סיכונים אמיתיים לפני המשך עבודה. קלט: ה-diff האחרון או הקבצים ששונו. פלט: Findings בלבד, ממוינים לפי חומרה, עם סיכונים התנהגותיים, רגרסיות, וחוסרים בבדיקות. Summary קצר רק בסוף אם צריך.',
            'אתה Review Agent. התמקד קודם בבאגים, רגרסיות וחורים בלוגיקה. רק אחר כך קריאות וניקיון. אם אין Findings, אמור שאין ואז ציין residual risks קצרים.',
            'אתה Review Agent. בדוק אם השינוי באמת תואם את @prd-v0.md או את ה-slice שהוגדר. פלט: מה נשבר, מה חסר, ואיזו בדיקה היית מוסיף לפני merge.',
          ],
          checklist: [
        ...
        },
        {
          ...
          prompts: [
            'אתה QA Agent. מטרה: להפוך acceptance criteria לתוכנית בדיקות קצרה. קלט: Acceptance criteria מתוך @prd-v0.md + ה-diff או המסך שבנינו. פלט: Happy path, 2 edge cases, 1 failure case, ומה מתאים לבדיקה אוטומטית עכשיו.',
            'אתה QA Agent. קח את "סיימנו כש..." מתוך @prd-v0.md והפוך אותם לתרחישי בדיקה שאפשר לבצע ידנית צעד-אחר-צעד.',
            'אתה QA Agent. תן לי plan מינימלי לשחרור: מה בודקים ידנית עכשיו, ומה שווה להפוך לטסט אוטומטי ראשון.',
          ],
          checklist: [
        ...
        },
        {
          ...
          prompts: [
            'אתה Debug Agent. מטרה: להסביר תקלה ולצמצם ניסוי-וטעייה. קלט: טקסט שגיאה מלא + stack trace + @ הקובץ החשוד. פלט: מה הראיות, ההשערה הכי סבירה, שחזור מינימלי, והתיקון הכי קטן שכדאי לנסות קודם.',
            'אתה Debug Agent. אל תנחש על כל הריפו. קח רק את הראיות שקיבלנו, הסבר מה הן אומרות, ותן צעד אחד לבדיקה לפני קוד.',
            'אתה Debug Agent. קלט: error log + @file. פלט: Probable cause, fix קטן אחד לנסות, ואיזה שינוי בפלט אמור להופיע אם צדקנו.',
          ],
          checklist: [
        ...
        },
        {
          ...
          badge: 'INTERMEDIATE',
          notes: [
            'אותו Agent יכול להיות מצוין — ועדיין להיכשל אם פתיח המשימה עמום או עמוס מדי.',
            'הפתיח הטוב לכל Task: מטרה, קבצים/קונטקסט עם @, פלט רצוי, מגבלות, ו«סיימנו כש…».',
            'קונטקסט נשמר פר Agent ופר Task: נושא חדש או משימה חדשה = שיחה חדשה או Agent אחר.',
            'השיעור הזה הוא ערכת עבודה: פרומפטים קצרים וחדים שאפשר להעתיק ולשנות.',
          ],
          prompts: [
            'אתה Product Agent. מטרה: להפוך רעיון למסמך PRD קצר. קונטקסט: @brief.md. פלט: Problem, Users, V1 scope, Flow, Metrics, Acceptance criteria, Risks. אם חסר מידע, שאל עד 5 שאלות לפני הכתיבה.',
            'אתה Builder Agent. מטרה: לבנות vertical slice אחד בלבד. קונטקסט: @prd-v0.md @architecture-handoff.md @project.mdc. פלט: 4 bullets לפני קוד, אחר כך diff קטן, ואז verification steps. אל תרחיב Scope מעבר לסלייס אחד.',
            'אתה Review Agent. מטרה: לבדוק את ה-diff האחרון. קונטקסט: הקבצים ששונו בלבד. פלט: Findings לפי חומרה, בלי מחמאות כלליות, עם המלצה על בדיקה חסרה אם צריך.',
            'אתה Debug Agent. מטרה: להסביר תקלה בלי לנחש. קונטקסט: טקסט השגיאה המלא + @ הקובץ החשוד. פלט: Evidence, hypothesis, smallest next step, expected result.',
            'אתה [שם ה-Agent]. מטרה: [מה רוצים]. קונטקסט: @[רק הקבצים הרלוונטיים]. פלט: [מסמך / diff / findings]. מגבלות: [מה לא לעשות]. סיימנו כש: [קריטריון בדיקה ברור].',
          ],
          checklist: [
            'בחרו משימה אחת מהמודול (PRD / Architecture / Build / Review) ופתחו לה Agent או שיחה נפרדת',
            'כתבו פתיח בן 4–5 שורות: מטרה, @ קבצים, פלט רצוי, מגבלות, «סיימנו כש…»',
            'הריצו את אותו Agent פעמיים: פעם עם פתיח עמום ופעם עם פתיח חד — והשוו את איכות התשובה',
            'אם עברתם למשימה אחרת, אל תמשיכו באותו קונטקסט; פתחו Agent אחר או שיחה חדשה',
            'שמרו לעצמכם 2–3 פרומפטים חזקים מהקורס — אלה ה־playbooks הראשונים שלכם',
          ],
        },
      ],
    },
