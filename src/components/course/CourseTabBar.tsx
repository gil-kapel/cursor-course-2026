...
import { Paperclip, StickyNote, FileText as TranscriptIcon, FileText, FileSpreadsheet, FileIcon, Presentation, Download, ExternalLink, Lightbulb, ScrollText } from 'lucide-react';
import { motion } from 'framer-motion';
import Card from '@/components/ui/Card';
import type { AttachedFile } from '@/data/types';

export type CourseTab = 'files' | 'notes' | 'transcript';
...
export default function CourseTabBar({ activeTab, onTabChange, files, notes, transcript }: CourseTabBarProps) {
      ...
      <div className="h-[17rem] overflow-y-auto scrollbar-ghost">
        <div className="p-6">
          ...
          {activeTab === 'transcript' && (
            <>
              {transcript ? (
                <div className="text-[0.8125rem] text-[#303150] leading-[1.8] whitespace-pre-line" style={{ fontFamily: 'var(--font-nunito), system-ui, sans-serif' }}>
                  {transcript}
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center h-[13rem]">
