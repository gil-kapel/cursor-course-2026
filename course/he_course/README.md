...
| 1.6 | Chat modes through **one example**: `contact.html` + link from `index.html` — **Ask** → **Plan** + **revise plan** (form CSS + in-page success, no `alert`) → **Agent** (browser check) → **Debug**; short **Max mode**; Shift+Tab where available | [module-01-intro-and-workbench/lesson-1.6-chat-modes](module-01-intro-and-workbench/lesson-1.6-chat-modes/README.md) |
