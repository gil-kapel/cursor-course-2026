...
**Given / When / Then** (optional but recommended): for each critical case, add a one-line triplet so engineers can map to code quickly:

| Given | When | Then |
|-------|------|------|
| User is logged out | They open `/dashboard` | Redirect to login |
| Cart has 1 item | They apply invalid coupon | Error message; cart unchanged |

**Map to automated tests (example: Vitest + React Testing Library)**  
After listing markdown cases, offer a **parallel** structure the dev agent can paste:

```typescript
// TC-F-001: [title] — maps to requirement X
...
Swap `describe`/`it` for `test` if that’s the project style. For E2E, reference **Playwright** `test.step` mirroring the same IDs (`TC-F-001`, etc.).

**Optional (property-based / fuzz):** for parsers, validators, or state machines, add 1–2 bullets suggesting **fast-check** (JS), **Hypothesis** (Python), or equivalent—only where random input is likely to find bugs; skip for trivial CRUD.
