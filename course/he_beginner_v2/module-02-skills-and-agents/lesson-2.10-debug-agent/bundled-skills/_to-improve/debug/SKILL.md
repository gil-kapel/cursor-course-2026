---
name: debug
description: Debug with evidence-first ritual (full traceback + log tail), then logs/DB/git; Cursor Composer friendly repro steps and smallest fix.
---
...
## Evidence ritual (ask first — Cursor / Composer)

Before spawning sub-tasks, ask the user to paste **in one message** (or attach files):

1. **Full error**: complete stack trace or compiler output — **not** a paraphrase.  
2. **Log tail**: last **~20–50 lines** around the failure (build, server, or browser console).  
3. **Repro**: minimal steps — “click X → enter Y → see Z”.  
4. **Environment**: OS, Node version if relevant, branch name.  
5. **Expected vs actual** — one sentence each.

**Composer workflow:** propose **one smallest change** at a time; after each fix, re-run the same repro steps. Avoid wide refactors during triage.
